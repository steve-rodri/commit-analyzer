...
import { ReportGenerationService, CommitStatistics } from '../../domain/services/ReportGenerationService'
...
/**
 * Use case for generating markdown reports from analyzed commits
 */
export class GenerateReportUseCase implements ICommandHandler<GenerateReportCommand, GenerateReportResult> {
  constructor(
  ...
  async handle(command: GenerateReportCommand): Promise<GenerateReportResult> {
    const { inputCsvPath, analyzedCommits, outputPath, includeStatistics = true } = command
    ...
    ConsoleFormatter.logInfo(`Found ${statistics.totalCommits} commits spanning ${statistics.yearRange.min}-${statistics.yearRange.max}`)
    ConsoleFormatter.logInfo(`Categories: ${statistics.categoryBreakdown.feature} features, ${statistics.categoryBreakdown.process} process, ${statistics.categoryBreakdown.tweak} tweaks`)

    // Generate and save the report
    ConsoleFormatter.logInfo('Generating condensed report...')
    await this.generateMarkdownReport(commits, statistics, outputPath, includeStatistics)
  ...
  private generateStatisticsSection(statistics: CommitStatistics): string {
    ...
    for (const [year, count] of Object.entries(statistics.yearlyBreakdown).sort()) {
  ...
  private generateYearSection(year: number, commits: AnalyzedCommit[]): string {
    ...
    const significantCommits = this.reportGenerationService.getSignificantCommits(commits)
    ...
    if (significantCommits.length > 0) {
      ...
      for (const commit of significantCommits.slice(0, 5)) { // Top 5 significant commits
